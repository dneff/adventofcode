 ...
 *
 * Finds the highest-scoring cookie recipe using exactly 100 teaspoons of ingredients.
 */
...
function scoreRecipe(ingredients, measures) {
  ...
  for (let i1 = 1; i1 < maxCount; i1++) {
    for (let i2 = 1; i2 < maxCount - i1; i2++) {
      for (let i3 = 1; i3 < maxCount - i1 - i2; i3++) {
        ...
        maxScore = Math.max(maxScore, scoreRecipe(ingredients, weights));
      }
    }
  }
