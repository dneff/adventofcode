...
def main():
    ...
    for guard in guards.keys():
        sleepiest_minute = max(guards[guard], key=guards[guard].get)
        sleepiest_count = guards[guard][sleepiest_minute]
        guard_top_minute.append((guard, sleepiest_minute, sleepiest_count))

    guard_top_minute.sort(key = lambda x: x[-1], reverse=True)
    printSolution(guard_top_minute[0][0] * guard_top_minute[0][1])
