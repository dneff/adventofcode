"""
Advent of Code 2016 Day 11 Part 2: Radioisotope Thermoelectric Generators

Problem: Same as Part 1, but with additional items discovered on the first floor:
- An elerium generator and elerium-compatible microchip
- A dilithium generator and dilithium-compatible microchip

These additional items must also be transported to the 4th floor following the same
safety constraints and elevator rules.
...
def parse_initial_state(filename):
    ...
    **Part 2 Addition**: In addition to the items from the input file, this adds
    two extra elements discovered in the cleanroom on the first floor:
    - Elerium: both microchip and RTG on floor 1
    - Dilithium: both microchip and RTG on floor 1
    ...
    # Part 2: Add the newly discovered elerium and dilithium items on floor 1
    element_positions['elerium'] = [1, 1]    # [chip_floor=1, rtg_floor=1]
    element_positions['dilithium'] = [1, 1]  # [chip_floor=1, rtg_floor=1]
...
def main():
    """
    Solve the Radioisotope Testing Facility puzzle (Part 2) using A* pathfinding.

    Part 2 includes the additional elerium and dilithium elements discovered in the
    cleanroom. Uses A* algorithm to find the minimum number of elevator moves needed
    to transport all RTGs and microchips to the top floor while maintaining safety.
    """
    ...
    # Parse the initial facility configuration (includes Part 2 additions)
    ...
    while not priority_queue.empty():
        ...
        if calculate_heuristic(current_items, num_floors) == 0:
            AoCUtils.print_solution(2, current_moves)
            return
