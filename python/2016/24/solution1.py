"""
Advent of Code 2016 - Day 24: Air Duct Spelunking (Part 1)

Problem: Guide a cleaning robot through an air duct system. The robot starts at
location 0 and must visit all numbered locations (1-7) at least once. Find the
shortest path (minimum steps) to accomplish this.
...
3. Try all permutations of visiting orders (starting from 0)
4. Return the shortest total path length
"""
...
def main():  # noqa: C901
    ...
    3. Find the shortest route visiting all locations starting from 0
    """
    ...
    # Try all permutations of visiting locations 1-7 starting from 0
    valid_routes = {}
    for visit_order in permutations(range(1, 8)):
        # Build the full route starting from location 0
        full_route = [0] + list(visit_order)
    ...
    # The answer is the minimum distance among all valid routes
    shortest_route_length = min(valid_routes.keys())
    AoCUtils.print_solution(1, shortest_route_length)
