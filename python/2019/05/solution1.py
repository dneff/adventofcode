...
def processProgram(prog, prog_input):
    ...
    while True:
        ...
        if instruction == 99:
            ...
            # input
            input_fn(prog[reader + 1], prog_input, prog)
            reader = advanceReader(reader, 2)
        elif instruction == 4:
            # output
            outputs.append(output_fn(prog[reader + 1], prog))
            reader = advanceReader(reader, 2)
        elif instruction == 5:
            # jump-if-true
    ...
    return outputs[-1] if outputs else prog[0]
...
def solve_part1():
    ...
    return processProgram(initial_prog, 5)
