...
def main():
    ...
    for seed in seeds:
        ...
        seeds[seed]['fertilizer'] = get_mapping(seeds[seed]['soil'], almanac['soil-to-fertilizer map'])
        seeds[seed]['water'] = get_mapping(seeds[seed]['fertilizer'], almanac['fertilizer-to-water map'])
        seeds[seed]['light'] = get_mapping(seeds[seed]['water'], almanac['water-to-light map'])
        seeds[seed]['temperature'] = get_mapping(seeds[seed]['light'], almanac['light-to-temperature map'])
        seeds[seed]['humidity'] = get_mapping(seeds[seed]['temperature'], almanac['temperature-to-humidity map'])
        seeds[seed]['location'] = get_mapping(seeds[seed]['humidity'], almanac['humidity-to-location map'])
